...
**Monorepo e tooling**
- Estrutura de diretórios conforme `@docs/architecture.md`: `apps/web`, `apps/api`, `infra/terraform`, `tests/e2e` (contendo somente `.gitkeep` estrutural nesta etapa), `docs`, `scripts`
...
- Configuração do Playwright, dependências E2E e scripts fictícios ou placeholders de `test:e2e` que sempre retornem exit code 0 (o diretório `tests/e2e` contém estritamente `.gitkeep` nesta etapa; configuração efetiva do Playwright e casos de teste E2E reservados para a etapa correspondente)
...
- Instalar Playwright, configurar suíte E2E ou criar script/placeholder fictício de `test:e2e` que mascare a ausência de testes E2E (o diretório `tests/e2e` contém estritamente `.gitkeep` nesta etapa; configuração do Playwright pertence à etapa correspondente)
