...
The repository SHALL be organized as a Node.js workspaces monorepo containing at minimum `apps/web`, `apps/api`, `tests/e2e`, `docs`, `scripts`, and `infra/terraform` directories. The `tests/e2e` directory SHALL exist as part of the monorepo skeleton from foundation containing strictly `.gitkeep`, with its workspace package configuration, Playwright setup, and E2E test suites deferred to the dedicated testing change.
...
#### Scenario: Structural tests/e2e directory
- **WHEN** the repository skeleton is inspected
- **THEN** the `tests/e2e` directory exists containing `.gitkeep`, with no fake or placeholder E2E test scripts
